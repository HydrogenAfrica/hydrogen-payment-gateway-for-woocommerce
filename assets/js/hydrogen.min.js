jQuery(function (e) {
  ...
  async function o() {
    ...
    e("form#payment-form, form#order_review")
      ...
      .val("");
    let o = Number(wc_hydrogen_params.amount);
    wc_hydrogen_params.meta_order_id;
    let i = window.location.href;
    function d() {
      k.click(), (window.location.href = "/cart");
    }
    (window.obj = {
      amount: o,
      ...
      Array.isArray(r()) &&
        ...
        e.isEmptyObject(t()) || (paymentData.metadata.custom_filters = t()));
    var s = new URLSearchParams(window.location.search),
      l = s.get("TransactionRef");
    s.get("key");
    ...
    if (-1 !== m) {
      var u = c.substring(m),
        p = u.indexOf("?");
      -1 !== p && (u = u.substring(0, p)), u.split("=")[1];
    }
    if (!l) {
      var f = window.location.href,
        h = f.lastIndexOf("?");
      if (-1 !== h)
        for (var y = f.substring(h + 1).split("&"), g = 0; g < y.length; g++) {
          var v = y[g].split("=");
          if ("TransactionRef" === v[0]) {
            l = v[1];
            break;
          }
        }
    ...
    if (l) {
      if (window !== window.parent)
        window.parent.postMessage({ TransactionRef: l }, "*");
      else {
        wc_hydrogen_params.meta_order_id,
        ...
        (b += "/cart/"), (transactionRef = l), w();
      }
    } else {
      async function $() {
        try {
          let e = await handlePgData(window.obj, window.token);
          console.log("return transaction ref", e),
            (transactionRef = e),
            window.innerWidth > 768
              ? (function e() {
                  ...
                  t &&
                    ...
                    (t.style.zIndex = "9999"));
                  let n = document.querySelector(".pgIframe");
                  n && (n.style.width = "27rem");
                })()
              : (function e() {
                  let a = document.getElementById("modal");
                  (a.style.height = "70%"),
    ...
    var k = document.querySelector(".modal .close");
    function w() {
      let a;
      (a = `
            <style>
        ...
        `),
        e("body").append(a),
        e("#wc-hydrogen-form").show();
      var t = wc_hydrogen_params.meta_order_id;
      e.ajax({
        ...
        beforeSend: function () {},
        success: function (e) {
          var a = wc_hydrogen_params.meta_order_id,
            t = window.location.href.replace(
              /\/checkout\/order-pay\/\d+\/.*/,
              ""
            );
          if (((t += "/cart/"), "90000" == e.statusCode)) {
            var r =
              "Your payment for order #" +
              a +
              " is successful and confirmed! Check your email or account for order details.";
            n(r, t);
          } else {
            var r =
              "Your payment for order #" +
              a +
              " was declined with status: Failed ! Click Ok.";
            n(r, t);
          }
        },
    ...
    k.addEventListener("click", w),
      window.addEventListener("message", function (e) {
        e.data && e.data.TransactionRef && k.click();
      });
  }
  e("#wc-hydrogen-form").hide(),
    o(),
