...
class WC_Gateway_Hydrogen_Three extends WC_Gateway_Custom_Hydrogen {
	 ...
	 * @var array
	 */
	public $hydrogen_settings;

	/**
	 * WC_Gateway_Hydrogen_Three constructor.
	 */
	public function __construct() {

		$this->id = 'hydrogen-three';

		$gateway_title = $this->get_option( 'title' );

		if ( empty( $gateway_title ) ) {
			$gateway_title = __( 'Three', 'woo-hydrogen' );
		}

		$this->method_title       = sprintf( __( 'Hydrogen - %s', 'woo-hydrogen' ), $gateway_title );
		$this->method_description = sprintf( __( 'Hydrogen provide merchants with the tools and services needed to accept online payments from local and international customers using Mastercard, Visa, Verve Cards and Bank Accounts. <a href="%1$s" target="_blank">Sign up</a> for a Hydrogen account, and <a href="%2$s" target="_blank">get your API keys</a>.', 'woo-hydrogen' ), 'https://hydrogen.com', 'https://dashboard.hydrogen.com/#/settings/developer' );
		...
		$this->hydrogen_settings = get_option( 'woocommerce_hydrogen_settings', '' );
		...
		$this->testmode = $this->hydrogen_settings['testmode'] === 'yes' ? true : false;
		...
		$this->test_public_key = $this->hydrogen_settings['test_public_key'];
		$this->test_secret_key = $this->hydrogen_settings['test_secret_key'];

		$this->live_public_key = $this->hydrogen_settings['live_public_key'];
		$this->live_secret_key = $this->hydrogen_settings['live_secret_key'];

		$this->saved_cards = $this->hydrogen_settings['saved_cards'] === 'yes' ? true : false;
