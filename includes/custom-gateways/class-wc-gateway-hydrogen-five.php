...
class WC_Gateway_Hydrogen_Five extends WC_Gateway_Custom_Hydrogen {
	...
	/**
	 * WC_Gateway_Hydrogen_Five constructor.
	 */
	public function __construct() {

		$this->id = 'hydrogen-five';

		$gateway_title = $this->get_option( 'title' );

		if ( empty( $gateway_title ) ) {
			$gateway_title = __( 'Five', 'woo-hydrogen' );
		}

		$this->method_title       = sprintf( __( 'Hydrogen - %s', 'woo-hydrogen' ), $gateway_title );
		$this->method_description = sprintf( __( 'Hydrogen Payment Gateway helps you process payments using cards and account transfers for faster delivery of goods and services. <a href="%1$s" target="_blank">Sign up</a> for a Hydrogen account, and <a href="%2$s" target="_blank">get your API keys</a>.', 'woo-hydrogen' ), '', '' );
